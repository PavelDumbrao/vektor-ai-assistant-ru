...
После перезапуска человек вручную меняет права Telegram Business: включает
**только «Отвечать на сообщения» (`can_reply`)**, а все остальные разрешения
оставляет выключенными. Receive-only профиль со всеми выключенными правами и
reply-only профиль с одним `can_reply` — два разных безопасных режима.
