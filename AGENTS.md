...
Для существующего VPS с несколькими профилями сначала прочитайте
`server/README.md` и `server/RESTORE.md`. Его актуальная программная база —
Hermes 0.21.0 + vektor3; используйте pinned archive и patch-series из
`modules/shared-runtime/releases/v0.21.0/vektor3.json`. Не подменяйте её
учебным снапшотом 0.20.0 ниже. Публикация GitHub не является командой деплоя.
...
Что даёт: ассистент видит разрешённую переписку человека в Telegram, расшифровывает голосовые, отвечает на вопросы «что было с Ивановым», «подведи итоги недели». Требования: PostgreSQL, сервер и явное подключение владельцем. Premium для подключения бота не требуется. Актуальное меню — «Настройки → Автоматизация чатов»; в старом интерфейсе «Telegram для бизнеса → Чат-боты».
...
7. **Руками человека** (Telethon не используем): Telegram → Настройки → Telegram для бизнеса → Чат-боты → вписать имя его бота → выбрать чаты и исключения. Для пассивного архива дополнительные права действий не нужны; рекомендуй не выдавать лишние права, но не требуй их отключения для работы сбора. Наличие прав ответа, просмотра подарков или других действий **не блокирует входящий архив** и само по себе не включает эти действия в Hermes.
8. Проверка: человек пишет кому-нибудь пару сообщений → в БД `select count(*) from passive_secretary.messages` растёт. Если записей нет, проверь активность подключения, owner allowlist, выбранные чаты, `capture_enabled`, PostgreSQL и очередь; не объявляй дополнительные права причиной остановки.

Сбор и исходящие действия — разные контуры. Для сбора сохраняются проверка
владельца, разрешённых источников, активности подключения и целостности DTO.
Некорректные метаданные прав отклоняются, но значения корректных `can_*` не
ограничивают сбор. Политика исходящих ответов ниже остаётся отдельной и строгой.

### Опциональные ответы от имени Telegram Business

По умолчанию пассивный секретарь остаётся receive-only и никому не пишет. Если
владелец отдельно попросил включить ответы, сначала выполни `status`, сохрани
все уже включённые флаги media/group/search и повтори активацию с теми же
флагами плюс:

```text
--enable-outbound-replies --confirm-outbound-policy --confirm-transcript-policy
```

После перезапуска человек вручную включает в Telegram Business «Ответы на
сообщения». Актуальный Telegram Desktop может автоматически оставить включённым
и «Чтение сообщений»: комбинация `can_reply=true` +
`can_read_messages=true` допустима, потому что «Вектор» не вызывает
`readBusinessMessage` и не ставит отметки о прочтении. Все остальные разрешения
должны быть выключены **для разрешения исходящих ответов**. Неподходящий для
ответа набор прав не отключает входящий архив. Режим без исходящих и reply-режим
с этой минимальной комбинацией — два разных режима действий.

Каждая отправка требует, чтобы текущее сообщение владельца в личном DM с
«Вектором» начиналось строго с `ОТПРАВИТЬ:` или `ОТВЕТИТЬ:`, после чего владелец
отдельно подтверждает точный текст и получателя кнопкой «Отправить один раз».
Ответ возможен только в личный Business-чат с входящим сообщением не старше
24 часов. Периодические, отложенные и самостоятельные Business-рассылки этим
модулем не поддерживаются; cron/internal event намеренно не получает право на
отправку.

## Несколько профилей на одном VPS: общий runtime

Для общего кода и зависимостей используй отдельный административный процесс
из `modules/shared-runtime/README.md`, а не доступ клиентов к домашней папке
другого пользователя. Личные `.hermes`, ключи, память, БД, плагины и рабочие
каталоги остаются раздельными. Разные версии кода нельзя молча выравнивать.

Допустим только root-owned набор в `/opt/vektor`, зарегистрированный в
`/opt/vektor/profiles/<owner>.json`. Проверяемая ссылка `hermes-agent` на такую
версию — отдельный поддерживаемый layout, а не разрешение любых симлинков.
Установщики секретаря сохраняют строгие проверки личных файлов и не изменяют
общие Python-зависимости от имени клиента. Готовый pinned `psycopg[binary]`
проверяется без `pip install`; обновляет общий runtime только администратор.

Не устанавливай Electron, все npm-workspaces и GUI devDependencies для простого
Telegram gateway. Нужные CLI устанавливаются централизованно с закреплённой
версией и раздельными сессиями. Не удаляй старые runtime без проверенного backup
и не останавливай посторонние helper-процессы ради освобождения Python.
