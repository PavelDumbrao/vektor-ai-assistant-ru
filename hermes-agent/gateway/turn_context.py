...
class TurnContext:
    ...
    # Trusted gateway-event provenance for security-sensitive pre-LLM hooks.
    # These stay separate from ``message``/``persist_user_message`` because
    # gateway preparation may prepend reply, attachment, or recovery context.
