...
def test_home_channel_notice_uses_russian_ui(monkeypatch):
    monkeypatch.setenv("HERMES_LANGUAGE", "ru")
    from gateway.run import _home_channel_notice_text

    notice = _home_channel_notice_text("telegram", "/sethome")
    assert "не задан домашний чат" in notice
...
def test_telegram_group_users_legacy_chat_ids_still_authorize(monkeypatch):
    ...
    """
    _clear_auth_env(monkeypatch)
    monkeypatch.setenv("TELEGRAM_GROUP_ALLOWED_USERS", "-1001234567890")
...
def test_telegram_group_users_mixed_sender_and_legacy_chat(monkeypatch):
    ...
    _clear_auth_env(monkeypatch)
    monkeypatch.setenv("TELEGRAM_GROUP_ALLOWED_USERS", "999,-1001234567890")
...
@pytest.mark.asyncio
async def test_unauthorized_dm_pairing_reply_uses_russian_ui(monkeypatch):
    _clear_auth_env(monkeypatch)
    monkeypatch.setenv("HERMES_LANGUAGE", "ru")
    config = GatewayConfig(
    ...
    result = await runner._handle_message(
        _make_event(Platform.TELEGRAM, "100000001", "100000001")
    )

    assert result is None
    adapter.send.assert_awaited_once()
    reply = adapter.send.await_args.args[1]
    assert "Код привязки" in reply
    assert "ABC12DEF" in reply
    assert "hermes pairing approve telegram ABC12DEF" in reply
