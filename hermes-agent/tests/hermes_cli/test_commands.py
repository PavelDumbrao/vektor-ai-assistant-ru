...
class TestGatewayHelpLines:
    ...
    def test_uses_russian_descriptions_when_configured(self, monkeypatch):
        monkeypatch.setenv("HERMES_LANGUAGE", "ru")
        lines = gateway_help_lines()
        help_line = [line for line in lines if line.startswith("`/help")]
        assert help_line == ["`/help` -- Показать доступные команды"]
...
class TestTelegramBotCommands:
    ...
    def test_uses_russian_descriptions_when_configured(self, monkeypatch):
        ...
        assert commands["help"] == "Показать доступные команды"
        assert commands["new"] == "Начать новый сеанс с чистой историей"
