(() => {
  ...
  async function loadMembers(){if(!current)return;const d=await request(`/v1/hermes/${current.profile}/members`);text("member-count",`${(d.members||[]).length} / ${d.limit||1}`);const r=$("members-list");r.textContent="";for(const m of d.members||[]){const c=document.createElement("div");c.className="card";const grants=(d.grants||[]).filter(g=>g.member_user_id==m.user_id);c.innerHTML=`<div class="row-between"><div><strong></strong><div class="muted">Участник · полная память</div></div><button class="danger-soft" data-remove-member="${m.user_id}">Удалить</button></div><div class="muted grants"></div>`;c.querySelector("strong").textContent=m.name||String(m.user_id);c.querySelector(".grants").textContent=grants.length?`Разрешения: ${grants.map(g=>g.tool_name).join(", ")}`:"Инструменты: разрешений пока нет";r.append(c)}for(const q of d.pending||[]){const c=document.createElement("div");c.className="card";const t=document.createElement("strong");t.textContent=`${q.member_name||q.member_user_id} просит: ${q.tool_name}`;const a=document.createElement("div");a.className="actions";for(const days of [1,7,30]){const b=document.createElement("button");b.className="secondary";b.dataset.grantMember=q.member_user_id;b.dataset.grantTool=q.tool_name;b.dataset.grantDays=days;b.textContent=`Разрешить ${days} дн.`;a.append(b)}c.append(t,a);r.append(c)}$("create-invite").disabled=(d.members||[]).length>=(d.limit||1)}
