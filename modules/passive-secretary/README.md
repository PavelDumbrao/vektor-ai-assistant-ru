...
## Входящий сбор не зависит от прав действий

Дополнительные Telegram Business rights, включая ответ, отметку о прочтении,
просмотр подарков, удаление сообщений и будущие корректные `can_*`, не отключают
сбор. Получение события не использует эти права действий. Core и normalizer
проверяют структуру метаданных, но не требуют receive-only/reply-only набора для
приёма. Правила владельца, выбранных чатов, активного подключения, digest и
tenant isolation сохраняются.

Исходящие ответы по-прежнему проходят отдельные opt-in и one-shot gates ниже.
Поле `reply_only` и архивное `telegram_can_reply` обозначают допустимость
узкого outbound-профиля; разрешённый сбор не делает их автоматически `true`.
Неподходящие для ответа права блокируют только действие, а не архив.
Ненужные права по-прежнему лучше не выдавать по принципу минимального доступа.

При обновлении устанавливайте согласованные версии core и normalizer.
Восстановление пропущенной старой версией истории — отдельная операция.
...
7. входящий сбор проверен и с пустыми, и с дополнительными корректными Telegram
   Business rights; посторонний owner, выключенное подключение и повреждённые
   метаданные по-прежнему отклоняются; для отдельного outbound-теста вручную
   включён `can_reply`, связанное
...
## Последовательный аудит больших периодов

`passive_secretary_activity` даёт компактный индекс без тел сообщений. `mode=days` возвращает карту активности по локальным календарным дням, а `mode=contacts` возвращает bounded/paginated список `source_ref` с количеством сообщений, incoming/outgoing, числом активных дней и first/last activity.

Для месячных и других больших аудитов модель сначала использует этот индекс, затем читает переписку ограниченными страницами через `passive_secretary_search`. Если search возвращает `has_more=true`, нужно продолжать по `next_cursor` до полного покрытия. Orchestration остаётся у самой модели: день → контакты → drill-down → проверка последующих сообщений → итог.
